  ...
  */
  ...
  var template = function anonymous(locals, attrs, escape, rethrow, merge) {
    ...
    with(locals || {}) {
      ...
      if (locals.footer) {
        buf.push('\n  <footer>\n    <nav id=""><button class="dial-number go-available btn btn-large btn-block" >Call</button><button class="dial-number-cancel go-available btn btn-large btn-block"  >Cancel</button></nav>\n  </footer>');
      }
      buf.push("\n</div>");
    }
    return buf.join("");
  ...
  Dialpad.prototype.getDispayableNumber = function() {
    var num = this.getNumber() + '',
      size = 16;
    return num.length > size ? num.slice(num.length - size, num.length) : num;
  };

  Dialpad.prototype.setNumber = function(number) {
    this.number = number;
    this.numberField.innerHTML = this.getDispayableNumber();
  };
